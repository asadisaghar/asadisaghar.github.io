...
Now lets merge the datasets andd then split them randomly into training, test, and validation sets). Now, when it comes to predicting the health stage of the engine, the training algorithm is actually aware of all the n (in this case 4) classe that we have defined.
